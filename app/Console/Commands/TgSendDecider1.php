...
class TgSendDecider1 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tgsenddecider1';
     ...
     * @var string
     */
    protected $description = 'TgSend Decider 1.';
    ...
    public function handle()
    {
        ...
        /*foreach($tg_data as $values){
            ...
            print "Call Of TgSend from Decider 1: ".$values->id."\n";
            sleep(2);
        }*/

        if(isset($tg_data[0])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[0]->id . "\n";
        ...
        if(isset($tg_data[1])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[1]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[1]->id]);
            sleep(2);
        }
        if(isset($tg_data[2])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[2]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[2]->id]);
            sleep(2);
        }
        if(isset($tg_data[3])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[3]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[3]->id]);
            sleep(2);
        }
        if(isset($tg_data[4])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[4]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[4]->id]);
            sleep(2);
        }
        if(isset($tg_data[5])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[5]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[5]->id]);
            sleep(2);
        }
        if(isset($tg_data[6])) {
            Artisan::call('tgsend', ['id' => $tg_data[6]->id]);
            print "Call Of TgSend from Decider 1: " . $tg_data[6]->id . "\n";
            sleep(2);
        }
        if(isset($tg_data[7])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[7]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[7]->id]);
            sleep(2);
        }
        if(isset($tg_data[8])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[8]->id . "\n";
            Artisan::call('tgsend', ['id' => $tg_data[8]->id]);
            sleep(2);
        }
        if(isset($tg_data[9])) {
            print "Call Of TgSend from Decider 1: " . $tg_data[9]->id . "\n";
