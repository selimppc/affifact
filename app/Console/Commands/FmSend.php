...
class FmSend extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            print "Old SE : ".$values->sender_email_id."\n";
            ...
            if($status == 'public' && $values->no_of_try >= 2){
                $cur_se = SenderEmailCheck::get_sender_email_public($values->campaign_id, $values->sender_email_id);
                ...
                // it would be inside the if as when sender email is changed then this would be changed accordingly
                $auth_token = $cur_se->auth_token ;
                $auth_code = $cur_se->auth_code;
                ...
                // it would be inside the elseif as when sender email is changed then this would be changed accordingly
                $auth_token = $cur_se->auth_token;
                $auth_code = $cur_se->auth_code;
                $reply_to = $cur_se->email;
                ...
                // it would be inside the else as when sender email is changed then this would be changed accordingly
